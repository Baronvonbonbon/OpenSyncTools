...
import { createRpcQueryExtension, createRpcClientClass, createRpcClientInterface, createRpcQueryHookInterfaces, createRpcQueryHookClientMap, createRpcQueryHooks, 
// grpc-gateway:
createGRPCGatewayQueryClass, createGRPCGatewayWrapperClass, 
//grpc-web:
createGrpcWebQueryClass, createGrpcWebQueryInterface, createMobxQueryStores, GetDesc, getMethodDesc, grpcWebRpcInterface, getGrpcWebImpl, createRpcClientImpl, } from '@subql/x-cosmology-ast';
            ...
            case 'grpc-web':
                allowedRpcServices.forEach(svcKey => {
                    if (proto[svcKey]) {
                        ...
                        const includeReactQueryHooks = c.proto.pluginValue('reactQuery.enabled') && isRefIncluded(c.ref, c.proto.pluginValue('reactQuery.include'));
                        ...
                        const mobxQueryStoreAst = createMobxQueryStores(ctx.generic, proto[svcKey]);
                ...
                allowedRpcServices.forEach(svcKey => {
                    if (proto[svcKey]) {
                        ...
                        instantOps.forEach((item) => {
                            ...
                            const methodKeys = getQueryMethodNames(bundlerFile.package, Object.keys(proto[svcKey].methods ?? {}), patterns, useCamelCase ? camel : String);
                        ...
                        // see if current file has been reactQuery enabled and included
                        const includeReactQueryHooks = c.proto.pluginValue('reactQuery.enabled') && isRefIncluded(c.ref, c.proto.pluginValue('reactQuery.include'));
                        const includeMobxHooks = c.proto.pluginValue('mobx.enabled') && isRefIncluded(c.ref, c.proto.pluginValue('mobx.include'));
                        if (includeReactQueryHooks) {
                            [].push.apply(asts, createRpcQueryHookInterfaces(ctx.generic, svc));
                        }
