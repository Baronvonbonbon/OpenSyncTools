...
import { getNestedProto } from '@subql/x-cosmology-proto-parser';
        ...
        if (context.proto.pluginValue('rpcClients.inline')) {
            ...
            allowedRpcServices.forEach(svcKey => {
                if (proto[svcKey]) {
                    ...
                    instantOps.forEach((item) => {
                        ...
                        context.body.push(createRpcClientInterface(context.generic, svc, item.className, methodKeys, nameMapping));
            ...
            if (proto.Msg) {
                ...
                instantOps.forEach((item) => {
                    ...
                    // get all query methods
                    const patterns = item.include?.patterns;
                    const methodKeys = getQueryMethodNames(bundlerFile.package, Object.keys(proto.Msg.methods ?? {}), patterns, useCamelCase ? camel : String);
                    if (!methodKeys || !methodKeys.length) {
                        return;
                    }
                    context.body.push(createRpcClientInterface(context.generic, proto.Msg, item.className, methodKeys, nameMapping));
