...
const getPackagesBundled = (store) => {
    ...
    Object.keys(pkgs).forEach(key => {
        if ((0, x_cosmology_proto_parser_1.isRefExcluded)((0, x_cosmology_proto_parser_1.createEmptyProtoRef)(key), store.options.prototypes?.excluded))
            return;
        const files = pkgs[key];
        dotty.put(objectified, key, {
            pkg: key,
            files: files.filter(file => {
                return !(0, x_cosmology_proto_parser_1.isRefExcluded)((0, x_cosmology_proto_parser_1.createEmptyProtoRef)(key, file.filename), store.options.prototypes?.excluded);
            })
        });
    });
    return objectified;
...
const createFileBundle = (options, pkg, filename, bundleFile, importPaths, bundleVariables) => {
    ...
    importPaths.push((0, x_cosmology_ast_1.importNamespace)(variable, (0, x_cosmology_utils_1.convertIfWinPath)(x_cosmology_utils_1.ToUnixPath, '/', rel)));
