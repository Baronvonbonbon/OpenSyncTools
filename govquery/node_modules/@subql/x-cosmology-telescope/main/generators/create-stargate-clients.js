...
const plugin = (builder, bundler) => {
    ...
    const ctx = new x_cosmology_ast_1.GenericParseContext(ctxRef, null, builder.options);
    ...
    bundler.registries.forEach(registry => {
        ...
        if (!rel.startsWith('.'))
            rel = `./${rel}`;
        const variable = (0, x_cosmology_utils_1.variableSlug)(registry.localname);
        registryVariables.push(variable);
        registryImports.push((0, x_cosmology_ast_1.importNamespace)(variable, rel));
    });
    bundler.converters.forEach(converter => {
        let rel = (0, path_1.relative)((0, path_1.dirname)(clientFile), converter.localname);
        if (!rel.startsWith('.'))
            rel = `./${rel}`;
        const variable = (0, x_cosmology_utils_1.variableSlug)(converter.localname);
        converterVariables.push(variable);
        converterImports.push((0, x_cosmology_ast_1.importNamespace)(variable, rel));
    });
    ...
    const aminos = (0, x_cosmology_ast_1.createStargateClientAminoRegistry)({
        context: ctx,
        aminos: converterVariables,
        aminoConverters: prefix + 'AminoConverters'
    });
    const protos = (0, x_cosmology_ast_1.createStargateClientProtoRegistry)({
        context: ctx,
        registries: registryVariables,
        protoTypeRegistry: prefix + 'ProtoRegistry'
    });
    const clientOptions = (0, x_cosmology_ast_1.createStargateClientOptions)({
        context: ctx,
        name: name + 'Options',
        protoTypeRegistry: prefix + 'ProtoRegistry',
        aminoConverters: prefix + 'AminoConverters'
    });
    const clientBody = (0, x_cosmology_ast_1.createStargateClient)({
        context: ctx,
        name,
    ...
    if (ctx.pluginValue("stargateClients.addGetTxRpc")) {
        getTxRpc = (0, x_cosmology_ast_1.createGetTxRpc)(ctx, txRpcName, name);
    }
