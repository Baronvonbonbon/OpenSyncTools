...
const plugin = (builder) => {
    ...
    const refs = builder.store.filterProtoWhere((ref) => {
        return (0, x_cosmology_proto_parser_1.isRefIncluded)(ref, {
            packages,
            protos
        }) && !(0, x_cosmology_proto_parser_1.isRefExcluded)(ref, builder.options.prototypes?.excluded);
    });
    ...
    const lcdast = (0, x_cosmology_ast_1.createAggregatedLCDClient)(context, services, 'QueryClient');
    ...
    const progImports = queryContexts.reduce((m, c) => {
        if (!(0, x_cosmology_proto_parser_1.isRefIncluded)(c.ref, {
            packages,
            protos
        })) {
            return m;
    ...
    const importStmts = (0, imports_1.getImportStatements)(localname, [...importsForAggregator, ...progImports]);
    const prog = []
        .concat(importStmts)
        .concat(lcdast);
