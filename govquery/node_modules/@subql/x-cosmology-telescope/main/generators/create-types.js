...
const plugin = (builder, bundler) => {
    ...
    bundler.contexts = baseProtos.map(ref => {
        ...
        if (context.proto.pluginValue('rpcClients.inline')) {
            const proto = (0, x_cosmology_proto_parser_1.getNestedProto)(context.ref.traversed);
            ...
            allowedRpcServices.forEach(svcKey => {
                if (proto[svcKey]) {
                    ...
                    context.body.push((0, x_cosmology_ast_1.createRpcClientInterface)(context.generic, proto[svcKey]));
                    instantOps.forEach((item) => {
                        ...
                        context.body.push((0, x_cosmology_ast_1.createRpcClientInterface)(context.generic, svc, item.className, methodKeys, nameMapping));
                    ...
                    context.body.push((0, x_cosmology_ast_1.createRpcClientClass)(context.generic, proto[svcKey]));
                    if (context.proto.pluginValue('rpcClients.extensions')) {
                        context.body.push((0, x_cosmology_ast_1.createRpcQueryExtension)(context.generic, proto[svcKey]));
                    }
                    else {
                        const env = context.proto.pluginValue('env');
                        if (env === 'v-next') {
                            context.body.push((0, x_cosmology_ast_1.createRpcClientImpl)(context.generic, proto[svcKey]));
                        }
                    }
                }
            });
            if (proto.Msg) {
                ...
                context.body.push((0, x_cosmology_ast_1.createRpcClientInterface)(context.generic, proto.Msg));
                instantOps.forEach((item) => {
                    let nameMapping = {
                        ...(0, x_cosmology_utils_1.swapKeyValue)(item.nameMapping?.All ?? {}),
                        ...(0, x_cosmology_utils_1.swapKeyValue)(item.nameMapping?.Msg ?? {})
                    };
                    // get all query methods
                    const patterns = item.include?.patterns;
                    const methodKeys = (0, x_cosmology_utils_1.getQueryMethodNames)(bundlerFile.package, Object.keys(proto.Msg.methods ?? {}), patterns, useCamelCase ? x_cosmology_utils_1.camel : String);
                    if (!methodKeys || !methodKeys.length) {
                        return;
                    }
                    context.body.push((0, x_cosmology_ast_1.createRpcClientInterface)(context.generic, proto.Msg, item.className, methodKeys, nameMapping));
                ...
                context.body.push((0, x_cosmology_ast_1.createRpcClientClass)(context.generic, proto.Msg));
                const env = context.proto.pluginValue('env');
                if (env === 'v-next') {
                    context.body.push((0, x_cosmology_ast_1.createRpcClientImpl)(context.generic, proto.Msg));
                }
            }
        }
        // build BASE file
