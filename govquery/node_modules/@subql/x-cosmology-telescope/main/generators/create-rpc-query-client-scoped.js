...
const makeRPC = (builder, bundler, rpc) => {
    ...
    builder.rpcQueryClients.forEach(file => {
        ...
        if (!(0, x_cosmology_proto_parser_1.isRefIncluded)((0, x_cosmology_proto_parser_1.createEmptyProtoRef)(file.package, file.proto), {
            packages,
            protos
        })) {
            return;
    ...
    const ctx = new build_1.TelescopeParseContext((0, x_cosmology_proto_parser_1.createEmptyProtoRef)(dir, localname), builder.store, builder.options);
    //based on rpc type to generate client from client factory
    let rpcast;
    switch (builder.options?.rpcClients?.type) {
        case "grpc-gateway":
            rpcast = (0, x_cosmology_ast_1.createScopedGrpcGatewayFactory)(ctx.proto, obj, "createGrpcGateWayClient"
        ...
        case "tendermint":
            ...
            rpcast = (0, x_cosmology_ast_1.createScopedRpcTmFactory)(ctx.proto, obj, methodName
            // 'QueryClientImpl' // make option later
            );
            break;
        case "grpc-web":
            rpcast = (0, x_cosmology_ast_1.createScopedGrpcWebFactory)(ctx.proto, obj, "createGrpcWebClient");
            break;
        default:
            break;
    }
    ...
    const importStmts = (0, imports_1.getImportStatements)(localname, [...(0, utils_1.fixlocalpaths)(imports)]);
    const prog = []
        .concat(importStmts)
        .concat(rpcast);
...
const makeAllRPCBundles = (builder, bundler) => {
    ...
    const refs = builder.store.getProtos().filter((ref) => {
        const proto = (0, x_cosmology_proto_parser_1.getNestedProto)(ref.traversed);
