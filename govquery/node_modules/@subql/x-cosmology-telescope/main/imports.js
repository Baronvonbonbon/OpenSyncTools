...
// __helpers__
const getImportStatements = (filepath, list) => {
    // swap helpers with helpers file...
    const modifiedImports = list.map(imp => {
        if (utils_1.UTIL_HELPERS.includes(imp.path)) {
            const name = imp.path.replace(/__/g, '');
            return {
                ...imp,
                path: (0, utils_1.getRelativePath)(filepath, `./${name}`)
            };
        }
        return imp;
    });
    ...
    return Object.entries(imports)
        ...
        if (namedImports.length) {
            m.push((0, x_cosmology_ast_1.importStmt)(namedImports.map(i => i.name), namedImports[0].path));
        }
        ...
        if (obj.type === 'import' && obj.path.startsWith('.')) {
            ...
            const appendSuffix = (obj, baseType) => {
                return {
                    ...obj,
                    orig: obj.name,
                    name: x_cosmology_ast_1.SymbolNames[baseType](obj.name),
                    importAs: x_cosmology_ast_1.SymbolNames[baseType](obj.importAs ?? obj.name),
                };
            };
            // MARKED AS NOT DRY [google.protobuf names]
            // TODO some have google.protobuf.Any shows up... figure out the better way to handle this
...
const aggregateImports = (context, allImports, filepath) => {
    ...
    if (context.options.useSDKTypes ||
        context.options.aminoEncoding?.enabled && !context.options.aminoEncoding?.useLegacyInlineEncoding) {
