...
exports.generateAbis = exports.getAbiNames = exports.prepareAbiJob = exports.prepareSortedAssets = exports.joinInputAbiName = void 0;
    ...
    for (const datasource of datasources) {
        ...
        for (const mappingHandler of datasource.mapping.handlers) {
            if (!mappingHandler?.filter)
                continue;
            if (mappingHandler.kind === types_flare_1.EthereumHandlerKind.Event) {
                const notMatch = mappingHandler.filter.topics.find((topic) => !abiEvents.includes(abi_1.EventFragment.fromString(topic).format()));
                if (notMatch)
                    topicIssues.push(notMatch);
            }
            if (mappingHandler.kind === types_flare_1.EthereumHandlerKind.Call) {
...
function prepareSortedAssets(datasources, projectPath) {
    const sortedAssets = {};
    datasources
        .filter((d) => !!d?.assets && ((0, project_1.isRuntimeDs)(d) || (0, project_1.isCustomDs)(d) || validateCustomDsDs(d)))
        .forEach((d) => {
