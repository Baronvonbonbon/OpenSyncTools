...
export const toAmino = {
    ...
    height(args) {
        ...
        const value = t.objectExpression([
            t.objectProperty(t.identifier(args.context.aminoCasingFn('revision_height')), t.optionalCallExpression(t.optionalMemberExpression(t.callExpression(t.identifier('omitDefault'), [
                t.memberExpression(memberExpressionOrIdentifier(args.scope), t.identifier('revisionHeight'))
            ...
            t.objectProperty(t.identifier(args.context.aminoCasingFn('revision_number')), t.optionalCallExpression(t.optionalMemberExpression(t.callExpression(t.identifier('omitDefault'), [
                t.memberExpression(memberExpressionOrIdentifier(args.scope), t.identifier('revisionNumber'))
            ]), t.identifier('toString'), false, true), [], false))
        ]);
    ...
    coinAmount(args) {
        ...
        switch (longType) {
            case 'BigInt':
                return t.memberExpression(memberExpressionOrIdentifier(args.scope), t.identifier('amount'));
            case 'Long':
            default:
                TypeLong.addUtil(args.context);
                return t.callExpression(t.memberExpression(t.callExpression(TypeLong.getFromValue(args.context), [
                    t.memberExpression(memberExpressionOrIdentifier(args.scope), t.identifier('amount'))
                ]), t.identifier('toString')), []);
        }
    },
    coin(args) {
        const value = t.objectExpression([
            t.objectProperty(t.identifier('denom'), t.memberExpression(memberExpressionOrIdentifier(args.scope), t.identifier('denom'))),
    ...
    typeArray({ context, field, currentProtoPath, scope, fieldPath, nested, isOptional }) {
        ...
        const expr = t.callExpression(t.memberExpression(memberExpressionOrIdentifier(scope), t.identifier('map')), [
