...
const rpcHookMethodInterface = (context, name, svc) => {
    ...
    return t.exportNamedDeclaration(t.tsInterfaceDeclaration(t.identifier(makeUseHookTypeName(name)), t.tsTypeParameterDeclaration([
        ...
        tsPropertySignature(t.identifier('request'), t.tsTypeAnnotation(t.tsTypeReference(t.identifier(requestType))), optional)
    ])));
};
/**
 ...
 * @returns {ParseResult} created AST
 */
export const createRpcQueryHooks = (context, service) => {
    // add imports
 ...
 * @returns {ParseResult} created AST
 */
export const createRpcQueryHookInterfaces = (context, service) => {
    const camelRpcMethods = context.pluginValue('rpcClients.camelCase');
    const methods = Object.keys(service.methods ?? {})
        .map(key => {
        const name = camelRpcMethods ? camel(key) : key;
 ...
 * @returns {ParseResult} created AST
 */
export const createRpcQueryHookClientMap = (context, service) => {
