...
export const grpcWebClientScaffold = () => {
    return [
...
export const grpcFuncArguments = () => {
    return [
        objectPattern([
...
export const grpcWebNewAwaitImport = (path, className) => {
...
export const grpcNestedImportObject = (obj, className) => {
    ...
    return t.objectExpression(keys.map(name => {
        return t.objectProperty(t.identifier(name), grpcNestedImportObject(obj[name], className));
    }));
};
export const createScopedGrpcWebFactory = (context, obj, identifier) => {
    context.addUtil('grpc');
    context.addUtil('NodeHttpTransport');
    return t.exportNamedDeclaration(t.variableDeclaration('const', [
        t.variableDeclarator(
        // createGrpcWebQueryClient
        t.identifier(identifier), t.arrowFunctionExpression(grpcFuncArguments(), t.blockStatement(grpcWebClientScaffold().concat(t.returnStatement(grpcNestedImportObject(obj, 'QueryClientImpl')))), true))
    ]));
};
export const createScopedGrpcWebMsgFactory = (obj, identifier, className) => {
    return t.exportNamedDeclaration(t.variableDeclaration('const', [
        t.variableDeclarator(t.identifier(identifier), t.arrowFunctionExpression(grpcFuncArguments(), 
        //
        t.blockStatement(grpcWebClientScaffold().concat(t.returnStatement(grpcNestedImportObject(obj, className)))), true))
    ]));
};
