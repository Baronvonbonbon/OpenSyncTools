import { classMethod, classProperty, commentBlock, identifier } from '../../../../utils';
...
const grpcGatewayPOSTServiceMethodDefinition = (name, svc, packageImport, leadingComments) => {
    ...
    const paramRequest = identifier('request', t.tsTypeAnnotation(t.tsTypeReference(t.identifier(requestType))), optional);
    ...
    return classMethod('method', t.identifier(name), [paramRequest, initRequest], // params
    body, returnReponseType(responseType), leadingComments, false, true);
};
...
const grpcGatewayQuerierMethodDefinition = (serviceName, context, name, svc, leadingComments) => {
    ...
    // ex: static Send(request: MsgSend)
    // paramRequest is an object representing everything in brackets here
    const paramRequest = identifier('req', t.tsTypeAnnotation(t.tsTypeReference(t.identifier(requestType))), false);
    // class method body (only return statement)
    const body = t.blockStatement([
    ...
    return classMethod('method', t.identifier(name), [paramRequest, headersInit], // params
    body, returnReponseType(responseType), leadingComments, false, false, // static
    false, true // async
    );
};
export const createGRPCGatewayWrapperClass = (context, service) => {
    ...
    return t.exportNamedDeclaration(t.classDeclaration(t.identifier(className), null, t.classBody([
        classProperty(t.identifier('url'), null, t.tsTypeAnnotation(t.tsStringKeyword()), [], false, false, true, "private"),
        t.classMethod('constructor', t.identifier('constructor'), [
            identifier('url', t.tsTypeAnnotation(t.tsStringKeyword()))
        ], t.blockStatement([
