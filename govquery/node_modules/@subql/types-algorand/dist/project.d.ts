import { BaseTemplateDataSource, IProjectNetworkConfig, CommonSubqueryProject, FileReference, ProjectManifestV1_0_0, BlockFilter, BaseHandler, BaseMapping, BaseDataSource, BaseCustomDataSource, SecondLayerHandlerProcessor_0_0_0, SecondLayerHandlerProcessor_1_0_0, DsProcessor } from '@subql/types-core';
 ...
 */
export type AlgorandTransactionHandler = AlgorandCustomHandler<AlgorandHandlerKind.Transaction, AlgorandTransactionFilter>;
/**
 ...
 */
export interface AlgorandCustomHandler<K extends string = string, F = Record<string, unknown>> extends BaseHandler<F, K> {
    /**
...
export interface AlgorandCustomDataSource<K extends string = string, M extends AlgorandMapping = AlgorandMapping<AlgorandCustomHandler>> extends BaseCustomDataSource<AlgorandHandler, M> {
    /**
...
export type SecondLayerHandlerProcessor<K extends AlgorandHandlerKind, F extends Record<string, unknown>, E, DS extends AlgorandCustomDataSource = AlgorandCustomDataSource> = SecondLayerHandlerProcessor_0_0_0<K, RuntimeHandlerInputMap, RuntimeFilterMap, F, E, DS, Indexer> | SecondLayerHandlerProcessor_1_0_0<K, RuntimeHandlerInputMap, RuntimeFilterMap, F, E, DS, Indexer>;
export type SecondLayerHandlerProcessorArray<K extends string, F extends Record<string, unknown>, T, DS extends AlgorandCustomDataSource<K> = AlgorandCustomDataSource<K>> = SecondLayerHandlerProcessor<AlgorandHandlerKind.Block, F, T, DS> | SecondLayerHandlerProcessor<AlgorandHandlerKind.Transaction, F, T, DS>;
export type AlgorandDataSourceProcessor<K extends string, DS extends AlgorandCustomDataSource<K> = AlgorandCustomDataSource<K>, P extends Record<string, SecondLayerHandlerProcessorArray<K, any, DS>> = Record<string, SecondLayerHandlerProcessorArray<K, any, DS>>> = DsProcessor<DS, P, Indexer>;
export type AlgorandProject<DS extends AlgorandDataSource = AlgorandRuntimeDataSource> = CommonSubqueryProject<IProjectNetworkConfig, AlgorandRuntimeDataSource | DS, BaseTemplateDataSource<AlgorandRuntimeDataSource> | BaseTemplateDataSource<DS>>;
export {};
