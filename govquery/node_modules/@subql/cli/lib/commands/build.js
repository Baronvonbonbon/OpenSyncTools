...
class Build extends core_1.Command {
    async run() {
        ...
        try {
            ...
            glob_1.default.sync(path_1.default.join(directory, 'src/test/**/*.test.ts')).forEach((testFile) => {
                const testName = path_1.default.basename(testFile).replace('.ts', '');
                buildEntries[`test/${testName}`] = testFile;
            });
            glob_1.default.sync(path_1.default.join(directory, 'src/tests/**/*.test.ts')).forEach((testFile) => {
                const testName = path_1.default.basename(testFile).replace('.ts', '');
