...
exports.createTypedObjectParams = exports.getParamsTypeAnnotation = exports.getPropertyType = exports.getType = exports.getTypeInfo = exports.detectType = exports.getTypeOrRef = exports.getTypeFromRef = void 0;
...
const getArrayTypeFromItems = (context, items) => {
    if (Array.isArray(items)) {
        ...
        return items.length > 0
            ? t.tsArrayType(t.tsArrayType((0, exports.getTypeOrRef)(items[0])))
            : t.tsArrayType(t.tsArrayType(t.tsAnyKeyword()));
        // }
    }
    else {
...
const getTypeInfo = (context, info) => {
    ...
    if (Array.isArray(info.anyOf)) {
        ...
        if (info.type.length !== 2) {
            throw new Error('please report this to maintainers (field type): ' +
...
function getPropertySignatureFromProp(context, jsonschema, prop, camelize) {
    ...
    if (Array.isArray(jsonschema.properties[prop].allOf)) {
        ...
        const unionTypes = jsonschema.properties[prop].allOf
            .map((el) => {
            if (el.title)
                return el.title;
            if (el.$ref)
        ...
        const isOptional = !jsonschema.required?.includes(prop);
        const unionTypes = jsonschema.properties[prop].oneOf
            .map((el) => {
            if (el.title)
                return el.title;
            if (el.$ref)
